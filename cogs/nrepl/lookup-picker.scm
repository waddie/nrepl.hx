...
;; Above this many symbols, fuzzy-match (which runs on the UI thread and is
;; poor on huge sets) is preceded by a cheap case-insensitive substring
;; pre-filter to bound the candidate set.
(define FUZZY-PREFILTER-THRESHOLD 2000)

(define (apply-filter state new-filter)
  "Filter symbols by fuzzy match and return new state.
   fuzzy-match returns a ranked subset; for very large symbol lists we substring
   pre-filter first to keep the UI responsive."
  (let* ([all-symbols (LookupPickerState-symbols state)]
         [filtered (if (string=? new-filter "")
                    all-symbols ; empty filter: keep the (already sorted) full list
                    (let ([candidates (if (> (length all-symbols) FUZZY-PREFILTER-THRESHOLD)
                                       (filter (lambda (sym)
                                                (string-contains-ci? sym new-filter))
                                         all-symbols)
                                       all-symbols)])
                      (fuzzy-match new-filter candidates)))])
